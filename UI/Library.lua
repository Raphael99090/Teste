...
function Library.Window.Create(title)
    ...
    -- Expõe a notificação de forma global para a Library
    Library.Notificar = function(self, t, d, time, type)
        Notify(t, d, time, type)
    end
    ...
    function WindowObj:CriarAba(icon, name)
        ...
        function TabObj:CriarPerfil()
            ...
            C.Size = UDim2.new(1, -5, 0, 160)
            C.BackgroundColor3 = Color3.fromRGB(20,20,25)
            Library.Utils.AddCorner(C, 12)
            Library.Utils.AddStroke(C, Library.Theme.ItemBg, 1)
            
            local Av = Instance.new("ImageLabel", C)
            Av.Size = UDim2.new(0,70,0,70)
            Av.Position = UDim2.new(0,15,0,45)
            Av.BackgroundTransparency = 1
            Library.Utils.AddCorner(Av, 100)
            Library.Utils.AddStroke(Av, Library.Theme.Accent, 2)
            
            task.spawn(function() 
                Av.Image = Players.LocalPlayer:GetUserThumbnailAsync(Players.LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size150x150) 
            end)
            ...
            local function Info(t, v, y, c) 
                ...
                L.TextColor3 = c or Library.Theme.Text
            ...
            Info("STATUS", "VIP ACTIVE", 60, Library.Theme.Success)
        end
...
-- Isso faz com que Interface.lua funcione sem precisar alterar nenhuma linha lá!
-- ==============================================================================
function Library:CriarJanela(title)
    return Library.Window.Create(title)
end
